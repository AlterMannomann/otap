...
OR p_otap_session.session_view_id             IS NULL]'
      ...
      ELSE
        ...
        END IF;
      END IF;
      otap_log.log('EXCEPTION -20099 ' || l_message, 'otap_objects.otap_session_verify', l_statement);
  ...
  BEGIN
    ...
    l_message := 'Current test settings' || otap_constants.OTAP_INTERNAL_LF ||
                 ...
                 'Name precedence: ' || CASE WHEN p_otap_session.name_precedence THEN otap_constants.OTAP_TEXT_TRUE_YES ELSE otap_constants.OTAP_TEXT_FALSE_NO END || otap_constants.OTAP_INTERNAL_LF ||
                 'Include packages: ' || CASE WHEN p_otap_session.include_packages THEN otap_constants.OTAP_TEXT_TRUE_YES ELSE otap_constants.OTAP_TEXT_FALSE_NO END || otap_constants.OTAP_INTERNAL_LF ||
                 'Persist: ' || CASE WHEN p_otap_session.persist_test THEN otap_constants.OTAP_TEXT_TRUE_YES ELSE otap_constants.OTAP_TEXT_FALSE_NO END || otap_constants.OTAP_INTERNAL_LF ||
  ...
  BEGIN
    ...
    THEN
      ...
      otap_log.log('EXCEPTION -20099 ' || l_message, l_script, 'p_schema IS NULL OR p_user IS NULL OR p_executor IS NULL');
      RAISE_APPLICATION_ERROR(-20099, l_message);
    END IF;
    -- check prefix length
