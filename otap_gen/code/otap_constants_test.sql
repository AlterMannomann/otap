...
-- create test statements for otap_constants package constants
  ...
  WITH base AS
     ...
     , vals AS
       (SELECT TRIM(SUBSTR(src, 1, cut_pos1)) AS const_name
             ...
             , src
             , '  l_return := otap_test.is_eq(otap_constants.' AS line_start
          FROM base
       )
SELECT line_start || const_name || ', ' || const_value || ', ''Verify package constant otap_constants.' || const_name || ''');'  AS test_line
  FROM vals
;
SPOOL OFF
EXIT
